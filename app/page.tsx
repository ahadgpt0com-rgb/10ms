...
export default function PinLoginScreen() {
  ...
  const [showSpecialVideo, setShowSpecialVideo] = useState(false);
  const [specialVideoUrl, setSpecialVideoUrl] = useState("https://cdn.discordapp.com/attachments/1433533756655927318/1501301051717062676/Screen_Recording_2026-05-06_004235.mp4?ex=69fb930f&is=69fa418f&hm=c83e499e471ad6e6dcc0398f78d1417d247546eed806d47468ade58f70e6c953&");

  useEffect(() => {
    const unsub = onSnapshot(doc(db, "settings", "global"), (docSnap) => {
      if (docSnap.exists() && docSnap.data().specialVideoLink) {
        setSpecialVideoUrl(docSnap.data().specialVideoLink);
      }
    });
    return () => unsub();
  }, []);
  ...
  useEffect(() => {
    if (showSpecialVideo) {
      const timer = setTimeout(() => {
        setShowSpecialVideo(false);
        // It will automatically route to dashboard because of the other useEffect
      }, 60000);
  ...
  useEffect(() => {
    if (user && !authLoading && !showWarningPopup && !showSpecialVideo) {
      router.push("/dashboard");
    }
  }, [user, authLoading, router, showWarningPopup, showSpecialVideo]);
  ...
  const handleNameSubmit = async (e: React.FormEvent) => {
    ...
    const isAsheka = name.trim().toLowerCase() === "asheka";
    if (isAsheka) {
      setShowSpecialVideo(true);
    }
  ...
  if (showSpecialVideo) {
    return (
      <div className="fixed inset-0 z-50 flex flex-col items-center justify-center bg-black/95 backdrop-blur-md animate-in fade-in duration-1000 p-4">
        
        <h2 className="text-3xl md:text-5xl font-serif text-pink-100 mb-8 tracking-widest animate-pulse font-light shadow-black drop-shadow-lg text-center">
          Only for you asheka ✨
        </h2>

        <div className="w-full max-w-4xl rounded-2xl overflow-hidden shadow-[0_0_40px_rgba(255,255,255,0.05)] border border-white/10 relative bg-black min-h-[300px] flex items-center justify-center group flex-col">
          {!iframeLoaded && (
            <div className="absolute inset-0 flex flex-col items-center justify-center z-0 text-white/70 space-y-4">
              <svg className="w-10 h-10 animate-spin text-pink-400" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg>
              <p className="text-sm">ভিডিও লোড হচ্ছে, কিছুক্ষণ অপেক্ষা করুন...</p>
            </div>
          )}
          <video
            src={specialVideoUrl}
            ...
            className={`w-full max-h-[65vh] object-contain relative z-10 transition-opacity duration-700 ${iframeLoaded ? 'opacity-100' : 'opacity-0'}`}
          />
        </div>

        <button 
          onClick={() => setShowSpecialVideo(false)}
          className="mt-8 px-8 py-3 rounded-full font-medium text-white/50 hover:text-white bg-white/5 hover:bg-white/10 transition-all border border-white/10"
        >
          Community তে প্রবেশ করুন
        </button>
      </div>
    );
  }
